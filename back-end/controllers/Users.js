...
export const createUser = async (req, res) => {
  ...
  try {
    ...
    const karyawansWithUser = defaultKaryawans.map((k) => ({
      ...k,
      userId: newUser.id, // gunakan newUser.uuid kalau relasinya pakai UUID
    }));

    // 4. Simpan ke DB
    await Karyawans.bulkCreate(karyawansWithUser);

    res.status(201).json("Registration successful with default karyawans");
  } catch (error) {
